...
export class AuthService {
    ...
    async login(ipAddress: string, userAgent: string, email: string,
                password?: string, code?: string, origin?: string): Promise<TokenResponse | TotpTokenResponse> {
        ...
        if (!user.prefersEmail) {
            throw new BadRequestException(NO_EMAILS);
        }
        ...
        if (code) {
            return this.loginUserWithTotpCode(ipAddress, userAgent, user.id, code, origin);
        }
        ...
        await this.checkLoginSubnet(ipAddress, userAgent, user.checkLocationOnLogin, user.id, origin);
    ...
    async register(ipAddress: string, _data: RegisterDto): Promise<Expose<User>> {
        ...
        if (!data.gender) {
            try {
                const prediction =
                    await axios.get<{ name: string; gender: 'male' | 'female'; probability: number; count: number; }>
                    (`https://api.genderize.io/?name=${data.name.split(' ')[0]}`);

                if (prediction.data.probability > 0.5 && prediction.data.gender === 'male') {
                    data.gender = 'MALE';
                }

                if (prediction.data.probability > 0.5 && prediction.data.gender === 'female') {
                    data.gender = 'FEMALE';
                }
            } catch (error) {
            }
        ...
        if (this.configService.get<boolean>('gravatar.enabled')) {
            for await (const emailString of [email, emailSafe]) {
                ...
                try {
                    const img = await got(`https://www.gravatar.com/avatar/${md5Email}?d=404`,
                        {responseType: 'buffer'});
        ...
        if (process.env.TEST) {
            ...
            await this.sendEmailVerification(email, false, origin);
        }
    ...
    async refresh(ipAddress: string, userAgent: string, token: string): Promise<TokenResponse> {
        if (!token) {
    ...
    async approveSubnet(ipAddress: string, userAgent: string, token: string): Promise<TokenResponse> {
        if (!token) {
            throw new UnprocessableEntityException(NO_TOKEN_PROVIDED);
        }

        const {id} = this.tokensService.verify<{ id: number }>(APPROVE_SUBNET_TOKEN, token);
    ...
    async enableMfaMethod(method: MfaMethod, userId: number, code: string): Promise<Expose<User>> {
    ...
    async loginWithTotp(ipAddress: string, userAgent: string,
                        token: string, code: string, origin?: string,): Promise<TokenResponse> {
    ...
    async loginWithEmailToken(ipAddress: string, userAgent: string, token: string): Promise<TokenResponse> {
    ...
    async resetPassword(ipAddress: string, userAgent: string, token: string,
                        password: string, ignorePwnedPassword?: boolean): Promise<TokenResponse> {
    ...
    async verifyEmail(ipAddress: string, userAgent: string, token: string, origin?: string): Promise<TokenResponse> {

        const {id} = this.tokensService.verify<{ id: number }>(EMAIL_VERIFY_TOKEN, token);
        ...
        const groupsToJoin = await this.prisma.group.findMany({
            where: {
                ...
                domains: {
                    some: {
                        isVerified: true,
                        domain: result.emailSafe.split('@')[1],
                    },
                },
            },
            select: {
    ...
    async mergeUsers(token: string): Promise<{ success: true }> {
        ...
        try {
            const result = this.tokensService
                .verify<{ baseUserId: number; mergeUserId: number; }>(MERGE_ACCOUNTS_TOKEN, token);
    ...
    private async loginUserWithTotpCode(ipAddress: string, userAgent: string, id: number,
                                        code: string, origin?: string,): Promise<TokenResponse> {
        ...
        for await (const backupCode of backupCodes) {
            if (await compare(code, backupCode.code)) {
                if (!usedBackupCode) {
                    ...
                    const locationName = [location?.city?.names?.en,
    ...
    private async loginResponse(ipAddress: string, userAgent: string, user: User): Promise<TokenResponse> {
    ...
    private async mfaResponse(user: User & { prefersEmail: Email; },
                              forceMethod?: MfaMethod): Promise<TotpTokenResponse> {
        const mfaTokenPayload: MfaTokenPayload = {
            type: user.twoFactorMethod,
            id: user.id,
        };

        const totpToken = this.tokensService.signJwt(MULTI_FACTOR_TOKEN, mfaTokenPayload,
    ...
    private async checkLoginSubnet(ipAddress: string, _: string, /* userAgent */
                                   checkLocationOnLogin: boolean, id: number, origin?: string): Promise<void> {
        ...
        for await (const item of previousSubnets) {
            if (!isApproved && await compare(subnet, item.subnet)) {
                isApproved = true;
            }
        }
